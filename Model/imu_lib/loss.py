...
class CLIPLoss(nn.Module):
    def __init__(self, initial_temperature=0.07):
        super().__init__()
        # 初始化可学习的温度参数（logit scale）
        self.logit_scale = nn.Parameter(torch.log(torch.tensor(1.0 / initial_temperature)))

    def forward(self, imu_features, skeleton_features):
        """
        计算CLIP对比损失
        Args:
            imu_features: 图像特征 [batch_size, embed_dim]
            skeleton_features: 文本特征 [batch_size, embed_dim]
        Returns:
            对比损失值
        """
        # 特征归一化
        ...
        # 计算相似度矩阵
        logit_scale = self.logit_scale.exp()  # 确保温度参数为正数
        logits_per_image = logit_scale * imu_features @ skeleton_features.T
        logits_per_text = logits_per_image.T

        # 创建标签（对角线为正样本）
        batch_size = imu_features.shape[0]
        labels = torch.arange(batch_size, device=imu_features.device)
